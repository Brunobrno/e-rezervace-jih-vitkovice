...
const CELL_SIZE = 60; // Size of cell
const ROWS = 14; // Num of rows
const COLS = 10; // Num of cols
...
const ReservationEditor = () => {
  ...
  const getCellCoords = useCallback((e) => {
    ...
    const x = clamp(Math.floor((e.clientX - rect.left) / CELL_SIZE), 0, COLS - 1);
    const y = clamp(Math.floor((e.clientY - rect.top) / CELL_SIZE), 0, ROWS - 1);
    return { x, y };
  }, [clamp]);

  // Check if two rectangles overlap
  const rectanglesOverlap = useCallback((a, b) => (
    a.x < b.x + b.w && 
    a.x + a.w > b.x && 
    a.y < b.y + b.h && 
    a.y + a.h > b.y
  ...
  const hasCollision = useCallback((newRect, ignoreIndex = -1) => (
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    if (e.target.classList.contains("resize-handle")) {
      const index = parseInt(e.target.closest(".reservation").dataset.index, 10);
    ...
    const resIndex = reservations.findIndex(
      r => coords.x >= r.x && coords.x < r.x + r.w && 
  ...
  // Delete reservation
  const handleDeleteReservation = useCallback((index) => {
  ...
  const handleStatusChange = useCallback((index, newStatus) => {
    setReservations(prev => prev.map((res, i) => 
      ...
      <div className="row">
        <div className="col-md-8">
          ...
          <div className="d-flex gap-2 mb-3">
            <span className="badge" style={{ backgroundColor: STATUS_COLORS.active }}>
              Aktivní
            </span>
            <span className="badge" style={{ backgroundColor: STATUS_COLORS.reserved }}>
              Rezervováno
            </span>
            <span className="badge" style={{ backgroundColor: STATUS_COLORS.blocked }}>
              Blokováno
            </span>
          </div>
          ...
          <div className="d-flex gap-2 mb-3">
            ...
            <button 
              ...
              onClick={() => {
                const dataStr = JSON.stringify(reservations);
                const blob = new Blob([dataStr], { type: 'application/json' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'reservace.json';
            ...
            {reservations.map((res, i) => (
              <div
                ...
                style={{
                  ...
                  fontSize: 12,
                  textAlign: "center",
                  transition: draggedIndex === i || resizingIndex === i 
                    ? "none" 
                    : "all 0.2s ease",
                  zIndex: 2,
                }}
